...
export default function RestaurantMenu() {
  ...
  // 4. FETCH MENU FROM FIRESTORE
  useEffect(() => {
    const fetchMenu = async () => {
      if (!id) return;
      try {
        const querySnapshot = await getDocs(collection(db, "restaurants", id, "menu"));
            ...
            <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all flex justify-between gap-4">
              ...
              <div className="relative w-32 h-32 flex-shrink-0">
                ...
                <button
                  ...
                  className={`absolute -bottom-3 left-1/2 -translate-x-1/2 shadow-lg rounded-lg font-bold border w-24 h-9 flex items-center justify-center transition-all active:scale-95 ${addedItems.includes(item.id)
